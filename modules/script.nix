...
(
  ''
    ...
    set +e
    # Log into Openbao using credentials from environmentFile
    until vaultOutput="$(bao write -format=json auth/approle/login role_id="$OPENBAO_ROLE_ID" secret_id=- <<< "$OPENBAO_SECRET_ID")"; do
      echo "Failed to login into Openbao, retrying"
      sleep 5
      [[ counter -eq $max_retry ]] && echo "Failed to login into Openbao" && exit 1
      ((counter++))
    done
    set -e
    jq '.auth.client_token = "redacted"' <<< "$vaultOutput"
    OPENBAO_TOKEN="$(jq -r '.auth.client_token' <<< "$vaultOutput")"
    export OPENBAO_TOKEN

  ''
  + optionalString (secretsKey != null) ''
    json_dump="$(bao kv get -format=json "${cfg.vaultPrefix}/${name}/${secretsKey}" || true)"
    if [[ -n "$json_dump" ]]; then
  ...
  + optionalString (environmentKey != null) ''
    json_dump="$(bao kv get -format=json "${cfg.vaultPrefix}/${name}/${environmentKey}" || true)"
    if [[ -n "$json_dump" ]]; then
  ''
  + (
    if quoteEnvironmentValues then
      ''
        jq -r '.data.data | to_entries[] | "${
          optionalString (environmentVariableNamePrefix != null) "${toUpper environmentVariableNamePrefix}_"
        }\(.key)=\"\(.value)\""' <<< "$json_dump" > "${secretsPath}/environment"
      ''
    else
      ''
        jq -r '.data.data | to_entries[] | "${
  ...
  + optionalString (extraScript != "") ''
    ...
    source <(jq -r '.data.data | to_entries[] | "export ${
      optionalString (environmentVariableNamePrefix != null) "${toUpper environmentVariableNamePrefix}_"
    }\(.key)=\"\(.value)\""' <<< "$json_dump")
