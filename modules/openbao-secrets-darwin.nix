...
in
{
  ...
  config = {
    launchd.daemons = mkMerge (
      [
        (flip mapAttrs' cfg.secrets (
          name: scfg:
          nameValuePair "${name}-secrets" {
            path = with pkgs; [
              getent
              jq
              openbao
            ...
            environment.OPENBAO_ADDR = cfg.openbaoAddress;
            # Needed to store openbao token
            ...
            serviceConfig = {
              ...
              StandardErrorPath = "/var/log/openbao-secrets/${name}-secrets-err.log";
              StandardOutPath = "/var/log/openbao-secrets/${name}-secrets-out.log";
            };
          }
        ))
      ]
