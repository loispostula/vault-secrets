        ...
        options = {
          ...
          environmentFile = mkOption {
            ...
            default =
              let
                rootDir =
                  if builtins.match ".*-darwin" pkgs.stdenv.hostPlatform.system != null then "/var/root" else "/root";
                fileName = if cfg.approlePrefix != null then "${cfg.approlePrefix}-${name}" else "${name}";
              in
              "${rootDir}/openbao-secrets.env.d/${fileName}";
            example = "/root/service.sh";
            description = ''
              Path to a file that contains the necessary environment variables for
              Openbao to log into an AppRole and pull data. Should define
              <literal>OPENBAO_ROLE_ID</literal> and
              <literal>OPENBAO_SECRET_ID</literal>.
            '';
          };
          ...
          environmentKey = mkOption {
            ...
            description = ''
              Openbao KV key under <literal>openbaoPrefix/namespace</literal> that
          ...
          secretsKey = mkOption {
            ...
            description = ''
              Openbao KV path under <literal>openbaoPrefix</literal>
          ...
          loginRetries = mkOption {
            ...
            default = 5;
            description = ''
              Number of attempts script will try to login into Openbao.
...
{
  openbao-secrets = {
    openbaoPrefix = mkOption {
      type = with types; str;
      default = "kv";
      description = ''
        Base Openbao KV path to prepend to all KV paths, including the mount point.
      '';
    };
    ...
    openbaoAddress = mkOption {
      type = with types; str;
      default = "https://127.0.0.1:8200";
      description = ''
        The address of the Openbao server, passed via <literal>OPENBAO_ADDR</literal> environment variable.
      '';
    };
